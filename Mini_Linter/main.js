...
// split the story into individual words and save them in a new array
const storyWords = story.split(' ');
// iterates and filter unnecessary words
const betterWords = storyWords.filter(word => { return  ! unnecessaryWords.includes(word)})

// filter overused words in an array
...
// count how many sentances are in the paragraph
let count = 0;
storyWords.forEach( word => {
  if( word[word.length - 1] === '.' || word[word.length-1] === '!') count +=1;
  } 
)

// answers
console.log(`The paragraph have ${storyWords.length} words.`)
console.log(`The paragraph have ${count} sentences.`)
console.log(`There are ${overusedWordsIndex.length} overused words. `);
